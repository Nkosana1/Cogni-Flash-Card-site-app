...
const studySlice = createSlice({
  ...
  reducers: {
    ...
    submitReview: (state, action: PayloadAction<{ quality: number }>) => {
      ...
      // Optimistic update: remove reviewed card from queue
      if (state.currentCardIndex < state.queue.length) {
        state.queue = state.queue.filter((_, index) => index !== state.currentCardIndex);
        if (state.currentCardIndex >= state.queue.length) {
          state.currentCardIndex = Math.max(0, state.queue.length - 1);
        }
      }
    },
    resetSession: (state) => {
